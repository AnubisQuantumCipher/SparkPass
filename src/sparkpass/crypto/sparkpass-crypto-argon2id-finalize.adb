...
with SparkPass.Crypto.Argon2id.HPrime; use SparkPass.Crypto.Argon2id.HPrime;

package body SparkPass.Crypto.Argon2id.Finalize with
   SPARK_Mode => On
is
   ...
   is
   begin
      --  For p=1 (SparkPass): Simply extract the last block
      --  C = Memory[lane_length - 1]
      --
      --  For Test_Medium: Memory[16,383]
      --  For Production:  Memory[131,071]
      --
      --  RFC 9106 Section 3.1.3:
      --    "If parallelism p > 1, the final block C is XOR of final blocks from all lanes"
      --    "If parallelism p = 1, C is just the final block of the single lane"

      Output := Memory (Active_Blocks_Per_Lane - 1);
   ...
   begin
      ...
      for Word_Index in Block_Word_Index loop
         ...
         pragma Loop_Invariant (
            (for all W in 0 .. Word_Index - 1 =>
               (for all B in 0 .. 7 =>
                  (W * 8 + B < 1024) and then
                  Output (1 + W * 8 + B)'Initialized)));
   ...
   begin
      ...
      --
      --  For SparkPass: C = Memory[Active_Blocks_Per_Lane - 1]
      ...
      pragma Assert (Block_Size_Bytes = 1024);
      pragma Assert (Final_Block_Bytes'Length <= 1024);  -- Precondition for H'
...
end SparkPass.Crypto.Argon2id.Finalize;
