...
--    - Parallelism p = 1 (single lane - simplification)
--    - Output length = 32 bytes (256 bits)
--    - Active_Blocks_Per_Lane = 16,384 (Test_Medium)
--    - Last block at index 16,383
--
--  **Implementation**:
...
private package SparkPass.Crypto.Argon2id.Finalize with
   SPARK_Mode => On
is
   ...
   procedure Block_To_Bytes (
      ...
      Pre    => Output'Length = Block_Size_Bytes and
                Output'First = 1,
      Post   => Output'Length = Block_Size_Bytes and
                Output'Initialized;
   ...
   procedure Finalize (
      ...
      Global => null,
      Pre    => Memory'First = 0 and
                Memory'Last = Active_Blocks_Per_Lane - 1 and
                Output_Length = 32 and  -- SparkPass specific
                Output'Length = 32 and
                Output'First = 1 and  -- For overflow proofs
                Output'Last = 32,  -- For range proofs
      Post   => Output'Length = 32;
