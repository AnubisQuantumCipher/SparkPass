...
package body SparkPass.Crypto.Argon2id.HPrime with
   SPARK_Mode => On
is
   ...
   procedure Compute_H_Prime (
      ...
      Out_Offset : Natural range 0 .. 1024 := 0;

      --  Bytes remaining to write
      Remaining : Natural range 0 .. 1024;
   ...
   begin
      ...
      if Output_Length <= 64 then
         ...
         loop
            ...
            pragma Assert (Out_Offset + Copy_Count <= 1024);  -- Needed for overflow proof
