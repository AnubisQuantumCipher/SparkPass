...
private package SparkPass.Crypto.Argon2id.Types is

   --  Ghost-level summary of the logical memory model.  Concrete arrays can
   --  refine this record when we introduce multiple lanes; proofs can reason
   --  about per-lane and aggregate properties without committing to the exact
   --  representation.
   --
   --  NOTE: Commented out due to forward reference issues - these constants
   --  are defined later in the file. Ghost code doesn't affect runtime.
   --
   --  type Memory_Profile is record
   --     Lane_Count    : Natural := 0;
   --     Blocks_Per_Ln : Natural := 0;
   --     Total_Blocks  : Natural := 0;
   --  end record;
   --  pragma Ghost (Memory_Profile);
   --
   --  function Default_Profile return Memory_Profile is
   --    (Lane_Count    => Parallelism,
   --     Blocks_Per_Ln => Active_Blocks_Per_Lane,
   --     Total_Blocks  => Active_Total_Blocks);
   --  pragma Ghost (Default_Profile);
   ...
   subtype Memory_Preset is SparkPass.Config.Argon2_Verification_Preset;

   --  Current verification mode (can be overridden for proof runs)
   Verification_Mode : constant Memory_Preset :=
     SparkPass.Config.Argon2_Verification_Mode;
   ...
   --  SparkPass uses p=1 (single lane) for simplicity and lower memory overhead
   Parallelism : constant Positive :=
     Positive (SparkPass.Config.Argon2_Parallelism);

   --  Iterations: Number of passes over memory (RFC 9106 Section 3.2)
   --  RFC 9106 Section 3.2: t ∈ [1, 2^32)
   --  SparkPass uses t=4 (from config)
   Iterations : constant Positive :=
     Positive (SparkPass.Config.Argon2_Iterations);
   ...
   function Total_Blocks (Preset : Memory_Preset) return Positive is
      (case Preset is
          when SparkPass.Config.Test_Small  => 64,        -- 64 KiB
          when SparkPass.Config.Test_Medium => 16_384,    -- 16 MiB
          when SparkPass.Config.Production  => 1_048_576) -- 1 GiB
   with
   ...
   --  For SparkPass: p=1, so only lane 0 exists
   ...
   ------------------------------------------------------------
   --  Type Invariants (Compile-Time Guarantees)
   ------------------------------------------------------------

   --  All these properties are guaranteed by the type system:
   --
   --  1. Lane_Index is always in [0, Parallelism-1]
   --  2. Block_Index is always in [0, Active_Blocks_Per_Lane-1]
   --  3. Segment_Index is always in [0, 3]
   --  4. Pass_Index is always in [0, Iterations-1]
   --  5. Block_Word_Index is always in [0, 127]
   --  6. Position fields are always valid indices
   --  7. Block size is exactly 1024 bytes
   --  8. U64_Mod arithmetic never overflows
   --
   --  These guarantees eliminate runtime checks and generate ZERO VCs
   --  for array bounds and arithmetic overflow.

end SparkPass.Crypto.Argon2id.Types;
