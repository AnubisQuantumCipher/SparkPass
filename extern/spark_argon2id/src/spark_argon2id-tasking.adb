...
package body Spark_Argon2id.Tasking is
   ...
   task type Lane_Worker (
      ...
      Mem         : access Memory_State;
      Active_Lanes              : Positive;
      Effective_Blocks_Per_Lane : Positive;
      Effective_Blocks_Per_Segment : Positive) is
   end Lane_Worker;
   ...
   begin
      loop
         ...
         Fill_Segment_For_Lane (Memory  => Mem.all,
                                 ...
                                 Effective_Blocks_Per_Lane => Effective_Blocks_Per_Lane,
                                 Effective_Blocks_Per_Segment => Effective_Blocks_Per_Segment);
         Controller.Notify_Done (Gen);
      end loop;
   end Lane_Worker;

   procedure Fill_Memory_Parallel (Memory : in out Memory_State;
                                   Active_Lanes : Positive;
                                   Effective_Blocks_Per_Lane : Positive;
                                   Effective_Blocks_Per_Segment : Positive) is
      Controller : aliased Segment_Controller (Active_Lanes);
      Mem_A   : aliased Memory_State := Memory;
      type Lane_Worker_Ref is access Lane_Worker;
      Workers : array (Natural range 0 .. Active_Lanes - 1) of Lane_Worker_Ref;
      Gen : Natural := 0;
   begin
      -- Create workers with proper discriminants
      for L_Int in Workers'Range loop
         declare
            L : constant Lane_Index := Lane_Index (L_Int);
         begin
            Workers (L_Int) := new Lane_Worker (Lane    => L,
   ...
   begin
      ...
      declare
         Active_Lanes : constant Positive := Positive (Integer (Params.Parallelism));
         Requested_Total_Blocks : constant Natural := Natural (Params.Memory_Cost);
         Requested_Q : Natural := (if Active_Lanes = 0 then 0 else Requested_Total_Blocks / Active_Lanes);
   ...
   begin
      ...
      declare
         Active_Lanes : constant Positive := Positive (Integer (Parallelism_Requested));
