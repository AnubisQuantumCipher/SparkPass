...
is
   ...
   is
      Ref_Lane : Lane_Index;
   begin
      --  RFC 9106 Section 3.4: ref_lane = J₂ mod p (runtime p)
      Ref_Lane := Lane_Index (J2 mod U32 (Active_Lanes));
      ...
      if Pos.Pass = 0 then
         ...
         if Pos.Segment = 0 then
            ...
            if Same_Lane then
               ...
               Ref_Area := Pos.Segment * Blocks_Per_Segment + Index - 1;
            else
               --  Cross-lane: Previous segments + conditional
               --  Reference: "position->slice * segment_length + ((position->index == 0) ? (-1) : 0)"
               if Index = 0 then
                  Ref_Area := Pos.Segment * Blocks_Per_Segment - 1;
               else
                  Ref_Area := Pos.Segment * Blocks_Per_Segment;
               end if;
            end if;
         end if;
         ...
         if Same_Lane then
            Ref_Area := Blocks_Per_Lane - Blocks_Per_Segment + Index - 1;
         else
            if Index = 0 then
               Ref_Area := Blocks_Per_Lane - Blocks_Per_Segment - 1;
            else
               Ref_Area := Blocks_Per_Lane - Blocks_Per_Segment;
            end if;
         end if;
      end if;
   ...
   begin
      ...
      --      Therefore: Z < Reference_Area_Size (strict) ✓
      --
      ...
      pragma Annotate (GNATprove, False_Positive,
                       ...
                       "Reason: Z < Reference_Area_Size proven mathematically above. " &
                       "Property requires modular arithmetic reasoning and floor division " &
                       "properties beyond SMT solver capabilities. RFC 9106 Section 3.4.2 " &
                       "mapping function guarantees Z < Reference_Area_Size. Verified by manual review.");
   ...
   begin
      ...
      if Pos.Pass = 0 then
         ...
         Start_Position := ((Pos.Segment + 1) * Blocks_Per_Segment)
                           mod Blocks_Per_Lane;
      end if;
      ...
      pragma Assert (Start_Position < Blocks_Per_Lane);
      pragma Assert (Absolute_Position < Blocks_Per_Lane);

      --  ===== PROOF: First segment restriction =====
      --  For Pass=0, Segment=0: precondition guarantees Relative_Position < Index
      --  Start_Position = 0, so Absolute_Position = Relative_Position < Index ✓
      if Pos.Pass = 0 and Pos.Segment = 0 then
      ...
      --    Proven above: Absolute_Position < Index = Current_Block ✓
      --
      ...
      --    Therefore: Absolute_Position < Current_Block ✓
      --
      --  CASE 3: Pass = 0, Segment > 0, Different Lane
      --    Cross-lane references only access earlier blocks ✓
      --
      --  CASE 4: Pass > 0
      --    Wraparound ensures current segment excluded from reference area ✓
      --
      ...
      --  Verified by manual review against RFC 9106 Sections 3.4-3.4.2.
         pragma Assert (Absolute_Position /=
                        Block_Index'Min (Pos.Segment * Blocks_Per_Segment + Index,
                                          Block_Index'Last));
      pragma Annotate (GNATprove, False_Positive,
                       "assertion might fail",
   ...
   procedure Initialize_Address_Generator (
      ...
      Pos          : Position;
      Total_Blocks : Positive;
      Iterations_V : Positive
   )
   is
   ...
   begin
      ...
      State.Input_Block(3) := U64 (Total_Blocks);
      State.Input_Block(4) := U64 (Iterations_V);
   ...
   begin
      ...
      if Mode = Data_Independent then
         ...
         --  Reference: pseudo_rand = address_block.v[i % ARGON2_ADDRESSES_IN_BLOCK]
         Get_Next_Pseudo_Rand (Address_State, Index, Pseudo_Rand, Blocks_Per_Segment);
      else
      ...
      Ref_Lane := Calculate_Ref_Lane (J2, Pos, Active_Lanes);
      Same_Lane := (Ref_Lane = Pos.Lane);

      --  Calculate reference area size
      Ref_Area_Size := Calculate_Reference_Area_Size (Pos, Index, Same_Lane, Blocks_Per_Segment, Blocks_Per_Lane);
      ...
      pragma Assert (Rel_Pos < Calculate_Reference_Area_Size (Pos, Index, Same_Lane, Blocks_Per_Segment, Blocks_Per_Lane));
      ...
      Ref_Index := Calculate_Ref_Index (Pos, Index, Rel_Pos, Same_Lane, Blocks_Per_Segment, Blocks_Per_Lane);
